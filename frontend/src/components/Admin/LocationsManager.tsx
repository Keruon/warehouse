...
export default function LocationsManager(): React.ReactElement {
  ...
  const [locationKindFilter, setLocationKindFilter] = useState<LocationKind | undefined>();
  ...
  const locationsQuery = useQuery({
    queryKey: ['admin-locations', page, pageSize, areaFilter, shelfFilter, locationKindFilter],
    queryFn: () => getLocationsPaged({ page, pageSize, areaId: areaFilter, shelfId: shelfFilter, locationKind: locationKindFilter }),
  });
  ...
  function openCreate(): void {
    ...
    form.resetFields();
    form.setFieldsValue({ binX: 0, binY: 0, isReserved: false, isActive: true, locationKind: 'Warehouse' });
    setModalOpen(true);
  }
  ...
  async function submit(values: LocationFormValues): Promise<void> {
    try {
      const payloadBase: CreateLocationRequest = {
        shelfId: values.locationKind === 'Warehouse' ? values.shelfId : undefined,
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Table<LocationResponse>
        ...
        columns={[
          ...
          {
            title: 'Kind',
            dataIndex: 'locationKind',
            render: (kind: LocationKind) => <Tag color={kind === 'Project' ? 'purple' : 'blue'}>{kind}</Tag>,
          },
      ...
      >
        <Form<LocationFormValues> form={form} layout="vertical" onFinish={submit}>
          <Form.Item
            name="locationKind"
            label="Location Kind"
            rules={[{ required: true, message: 'Location kind is required.' }]}
          ...
          <Space style={{ width: '100%' }} align="start">
            <Form.Item
              noStyle
              shouldUpdate={(prev, next) => prev.locationKind !== next.locationKind}
            >
              {() => {
                const selectedKind = form.getFieldValue('locationKind') as LocationKind | undefined;
                const isWarehouse = selectedKind !== 'Project';
                return (
                  <Form.Item
                    name="areaId"
                    label="Area"
                    rules={isWarehouse ? [{ required: true, message: 'Area is required.' }] : []}
                    style={{ minWidth: 220 }}
                  >
                    <Select
                      options={(areasQuery.data ?? []).map((a) => ({ label: `${a.name} (${a.code})`, value: a.id }))}
            ...
            <Form.Item
              noStyle
              shouldUpdate={(prev, next) => prev.locationKind !== next.locationKind || prev.areaId !== next.areaId}
            >
              {() => {
                const selectedKind = form.getFieldValue('locationKind') as LocationKind | undefined;
                const isWarehouse = selectedKind !== 'Project';
                return (
                  <Form.Item
                    name="shelfId"
                    label="Shelf"
                    rules={isWarehouse ? [{ required: true, message: 'Shelf is required.' }] : []}
                    style={{ minWidth: 220 }}
                  >
                    <Select
